# Hey, I'm Harsh 👋  
💻 Computer Science Student | Tech Enthusiast | Everyday Learner

---

### 🌱 About Me
🎓 Pursuing a degree in Computer Science.  
🧠 Passionate about Full-Stack Development, AI, and Open Source.  
💬 Currently exploring advanced projects in Web, AI, and systems design.  
🏆 On a journey to master DSA, Competitive Programming, and real-world engineering.

---

### 🔭 Highlight Projects
<!-- 🚀 **Virtual Try-On for Eyeglasses (Lenskart Clone)** – JS, WebGL  
Try glasses in real-time using face detection and 3D rendering.  
-->
💬 **WhatsApp Exchange Portal for ZACH LIFESTYLE** – Node.js, Shopify API  
Built a full-stack exchange request system integrated with Shopify data.

🧠 **FocusSphere (Digital Wellbeing App)** – React Native  
Helps reduce screen time and build healthy phone habits.

---

<!-- ### 🛠 Skills
**Languages:**  
C++, JavaScript, TypeScript, Python, SQL  

**Frameworks / Tools:**  
Node.js, Express.js, React.js, Next.js, EJS, MySQL, MongoDB, Docker, Linux, Git, Nginx  

**Learning:**  
WebGL, AI integrations, and scalable backend architecture -->

### 🛠 Skills & Stack

**Languages**
<p>
  <img src="https://skillicons.dev/icons?i=cpp,js,ts,python" />
</p>

**Frontend**
<p>
  <img src="https://skillicons.dev/icons?i=react,nextjs,tailwind,html,css" />
</p>

**Backend**
<p>
  <img src="https://skillicons.dev/icons?i=nodejs,express,graphql" />
</p>

**Databases & ORMs**
<p>
  <img src="https://skillicons.dev/icons?i=mongodb,postgres,mysql,redis,prisma" />
</p>

**DevOps & Cloud**
<p>
  <img src="https://skillicons.dev/icons?i=docker,kubernetes,aws,nginx,github,linux" />
</p>

**Tools**
<p>
  <img src="https://skillicons.dev/icons?i=git,kafka,grafana,postman,vscode" />
</p>

**📚 Currently Learning**
<p>
  <img src="https://skillicons.dev/icons?i=react,ts,postgres,docker,nextjs" />
</p>

---

<!-- ### 🏆 Achievements
🎯 Consistent 5★ performer on platforms like LeetCode and Codeforces *(your rating here if any)*  
🥇 Science Exhibition Project – Qualified for State Level *(if applicable)*  
🌐 Built multiple production-ready web apps and API integrations  

--- -->

### 👨‍💻 Experience
💼 **Freelance / Personal Projects** – Building production-grade web apps  
🧩 **ZACH LIFESTYLE (Client Project)** – Shopify-integrated exchange system  
🧠 **Personal AI/ML Experiments** – Working on ML-driven apps for social well-being  

---

### 📫 Contact Me
💼 [LinkedIn]([https://www.linkedin.com/in/harsh-yadav-76a16b284/])  
📧 harshyadav68529@gmail.com  
⭐ Feel free to explore my repositories or collaborate on exciting projects!

---
### 📊 GitHub Stats
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=HarshYadav68&show_icons=true&theme=dark&hide_border=true" />
  <br/>
  <img src="https://streak-stats.demolab.com?user=HarshYadav68&theme=dark&hide_border=true" />
  <br/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=HarshYadav68&layout=compact&theme=dark&hide_border=true" />
</p>




<!--
**HarshYadav68/HarshYadav68** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
